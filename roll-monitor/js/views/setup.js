  ...
  SetupView.showAudit = function (id, state) {
    ...
    const timeInfo = r.durationMin !== null
      ? `قانون زمان: دستگاه ${U.faNum(r.machineNumber)} + متراژ ${U.faNum(r.standardLength)} →
         ...
         مانده زمان ستاپ = ${U.faNumPlain(r.durationMin)} × ${U.faNum(r.rollCount)} = <b>${U.faHMM(r.remainingSetupTime)}</b> ·
         مانده زمان موجودی = ${U.faNumPlain(r.durationMin)} × (${U.faNum(r.rollCount)} − ${U.faNum(r.metallized)})${r.rollCount - r.metallized < 0 ? ' → پایهٔ منفی → ۰' : ''} = <b>${U.faHMM(r.remainingProducedTime)}</b>`
