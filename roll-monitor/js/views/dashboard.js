...
(function () {
  ...
  Dashboard.render = async function (state) {
    ...
    document.getElementById('kpi-metallized').textContent = U.faNum(s.metallizedCount);
    document.getElementById('kpi-metallized-meta').innerHTML =
      `تخصیص‌یافته به ستاپ‌ها: ${U.faNum(s.metallizedAllocated)} · ` +
      (s.metallizedUnallocated > 0
        ? `⚠ تخصیص‌نیافته: ${U.faNum(s.metallizedUnallocated)}`
        : 'بدون مازاد');

    document.getElementById('kpi-setup-total').textContent = U.faNum(s.totalSetupRolls);
        ...
        UI.infoModal(warning.title, `
          ...
          <div class="table-wrap table-scroll" style="max-height:420px">
            <table class="data-table">
              <thead><tr><th>ستاپ</th><th>دستگاه</th><th>عرض</th><th>ضخامت</th><th>متراژ استاندارد</th><th>تعداد</th><th>خام</th><th>متالایز</th><th>مغایرت</th></tr></thead>
              <tbody>${rows.map((r) => `
                <tr>
