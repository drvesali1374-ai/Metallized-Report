...
(function () {
  ...
  Setups.validate = function (input, rules, allSetups, editId = null, options = {}) {
    ...
    if (!setupDateParsed) {
      ...
      // در ثبت جدید: تاریخ ستاپ موجود به ارث می‌رسد و قفل است (مثل ضخامت §22)
      const sameSetupD = allSetups.filter(
        (s) => s.setupNumber === setupNumber && (editId === null || s.id !== editId) && s.setupDate
      );
      if (sameSetupD.length && sameSetupD[0].setupDate !== setupDateParsed.normalized) {
        errors.setupDate = `تاریخ ستاپ ${U.faNum(setupNumber)} قبلاً ${U.faJalali(sameSetupD[0].setupDate)} ثبت شده و باید یکسان باشد (ارث‌بری تاریخ ستاپ).`;
      }
    }
  ...
  Setups.update = async function (id, input, options = {}) {
    ...
    // در ویرایش، تغییر ضخامت (§23) و تاریخ (نسخهٔ ۲٫۸) با آبشار مجاز است
    ...
    await db.transaction('rw', db.setupRolls, async () => {
      ...
      // آبشار ضخامت و تاریخ به سایر رکوردهای همان شماره ستاپ (§23 + نسخهٔ ۲٫۸) — تراکنشی
  ...
  Setups.cascadeInfo = function (id, newThickness, newDate) {
    return db.transaction('r', db.setupRolls, async () => {
      ...
      const others = all.filter((s) => s.id !== id && s.setupNumber === target.setupNumber);
  ...
  Setups.BULK_GLOBAL_FIELDS = ['setupNumber', 'thickness', 'setupDate'];
   ...
   * خروجی: { ok, globalErrors, records, total, allowedRows }
  ...
  Setups.validateBulkDraft = async function (common, drafts, options = {}) {
    ...
    /* --- تاریخ ستاپ Batch (نسخهٔ ۲٫۸): یک فیلد تاریخ برای کل دیتای پیست‌شده ---
       اگر ستاپِ سراسری قبلاً تاریخ ثبت‌شده داشته باشد، همان تاریخ به‌ارث می‌رسد
       و قفل است (دقیقاً مانند منطق ضخامت — یک تاریخ برای همهٔ رکوردهای همان
       شماره ستاپ)؛ در غیر این صورت تاریخ Batch الزامی و باید شمسیِ معتبر باشد. */
    const commonSetupNo = U.parseInt(common.setupNumber);
    const inheritedDate = commonSetupNo !== null
      ? (allSetups.find((s) => s.setupNumber === commonSetupNo && s.setupDate) || null)
      : null;
    let batchDate = null;
    if (inheritedDate) {
      batchDate = U.parseJalaliDate(inheritedDate.setupDate);
    } else {
    ...
    for (const draft of drafts) {
      ...
      // کلید یکتای این ردیف (نمایشی — ترتیب قطعات الگو بی‌اثر)
    ...
    return {
      ...
      dateInherited: !!inheritedDate,                          // نسخهٔ ۲٫۸ — تاریخ از رکورد موجود همان ستاپ به‌ارث رسیده
      inheritedDateValue: inheritedDate ? inheritedDate.setupDate : null,
