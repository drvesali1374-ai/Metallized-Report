  ...
  async function calculateProductionState() {
    ...
    const setupReport = setupRolls.map((setup) => {
      ...
      /* --- استخراج مدت زمان از قوانین زمان (دستگاه + متراژ استاندارد) ---
         ...
         مانده زمان موجودی = مدت زمان × (تعداد − متالایز)   ← نسخهٔ ۲٫۵
         (پایهٔ منفی — بیش از سفارش متالایزشده — به صفر محدود می‌شود)
      ...
      // نسخهٔ ۲٫۵: (تعداد − متالایز) × مدت — از جدول «تعریف قانون زمان متالایز (هر دستگاه)»
      const inventoryBase = Math.max(0, setup.rollCount - metallizedAllocated);
      const remainingProducedTime = durationMin !== null ? durationMin * inventoryBase : null;
      ...
      return {
        ...
        remainingInventoryCount: inventoryBase,   // پایهٔ زمان موجودی = تعداد − متالایز (نسخهٔ ۲٫۵)
        audit: {
