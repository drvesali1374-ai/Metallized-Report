...
(function () {
  ...
  async function calculateProductionState() {
    ...
    const setupReport = setupRolls.map((setup) => {
      ...
      /* وضعیت ستاپ (نسخهٔ ۲٫۸): «تمام شده» وقتی مانده زمان ستاپ = ۰ و
         «باقی مانده» وقتی > ۰؛ بدون قانون زمان → null (نامشخص). */
      const timeStatus = remainingSetupTime === null
        ? null
        : (remainingSetupTime === 0 ? 'done' : 'remaining');
      ...
      return {
        ...
        timeStatus,                                // 'done' | 'remaining' | null (نسخهٔ ۲٫۸)
