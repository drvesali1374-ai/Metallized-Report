...
(function () {
   ...
   *   تعداد رکوردها، کل عرض‌های مادر، کل رول‌های الگوی برش،
   *   جمع عدد ست‌ها، رول‌های یونیک مادر، رول‌های یونیک الگو.
   */
  SetupView.bulkSummaryStats = function (drafts) {
    let mothers = 0, pieces = 0, sets = 0;
    const motherSet = new Set();
    const patternSet = new Set();

    for (const d of drafts) {
      const occ = d.occurrences || 1;
      mothers += occ;                                        // هر occurrence یک مادر است
      const parsed = Setups.parsePattern(d.patternText);
      if (parsed.ok) {
        pieces += parsed.parts.length * occ;                 // هر قطعهٔ الگو یک رول برش است
        if (parsed.parts.length) patternSet.add(Setups.normalizePatternKey(d.patternText));
      }
      if (d.rollCount !== null) sets += d.rollCount;         // عدد ست‌ها (انتهای ردیف‌های فرد)
      const w = U.parseNumber(d.motherToken);
      if (w !== null) motherSet.add(w);
    }

    return {
      count: drafts.length, mothers, pieces, sets,
      uniqueMothers: motherSet.size, uniquePatterns: patternSet.size,
    };
  };

  /** رندر خلاصهٔ زنده در #bulk-summary */
  SetupView.renderBulkSummary = function () {
    ...
    el.innerHTML = `
      ...
      <span class="chip">عرض‌های مادر: ${U.faNum(s.mothers)}</span>
      <span class="chip">رول‌های الگوی برش: ${U.faNum(s.pieces)}</span>
      <span class="chip chip-strong">جمع عدد ست‌ها: ${U.faNum(s.sets)}</span>
      <span class="chip chip-ok">یونیک مادر: ${U.faNum(s.uniqueMothers)}</span>
      <span class="chip chip-ok">یونیک الگو: ${U.faNum(s.uniquePatterns)}</span>`;
  };
